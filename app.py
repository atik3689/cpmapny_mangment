...
class Admin(db.Model):  # Admin Signup
    ...
    def authenticate(email, password):   #admin login
        return Admin.query.filter_by(email=email, password=password).first()

class TeamLeader(db.Model):    #team leader signup
    id = db.Column(db.Integer, primary_key=True)
    first_name = db.Column(db.String(255), nullable=False)
    last_name = db.Column(db.String(255), nullable=False)
    address = db.Column(db.Text, nullable=False)
    ...
    def authenticate(email, password):   #team leader login
        return TeamLeader.query.filter_by(email=email, password=password).first()

class Recruiter(db.Model):      #recruiter signup
    id = db.Column(db.Integer, primary_key=True)
    first_name = db.Column(db.String(255), nullable=False)
    last_name = db.Column(db.String(255), nullable=False)
    address = db.Column(db.Text, nullable=False)
    ...
    @staticmethod
    def authenticate(email, password):       #recruiter login
        return Recruiter.query.filter_by(email=email, password=password).first()

class Company(db.Model):        #add company
    id = db.Column(db.Integer, primary_key=True)
...
class AppUp(db.Model):
    ...
    def _init_(self, job_app_id, status, remark):
